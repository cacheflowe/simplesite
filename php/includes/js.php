<!-- Simplesite libs -->
<script src="/simplesite/js/vendor/fetch.js"></script>
<script src="/simplesite/js/vendor/page.js"></script>
<script src="/simplesite/js/vendor/medium-zoom.min.js"></script>
<script src="/simplesite/js/vendor/notyf.min.js"></script>
<script src="/simplesite/js/haxademic/app-store-.es6.js"></script>
<script src="/simplesite/js/haxademic/dom-util.es6.js"></script>
<script src="/simplesite/js/haxademic/easy-scroll.es6.js"></script>
<script src="/simplesite/js/haxademic/lazy-image-loader.es6.js"></script>
<script src="/simplesite/js/haxademic/lightbox.es6.js"></script>
<script src="/simplesite/js/simplesite/area-model.es6.js"></script>
<script src="/simplesite/js/simplesite/base-view.es6.js"></script>
<!-- Custom -->
<script src="/simplesite/js/vendor/embetter.js"></script>
<script src="/js/views/base-site-view.es6.js"></script>
<script src="/js/views/count-view.es6.js"></script>
<script src="/js/views/customtext-view.es6.js"></script>
<script src="/js/views/dashboard-view.es6.js"></script>
<script src="/js/views/json-editor-view-.es6.js"></script>
<script src="/js/views/json-editor-view-config.es6.js"></script>
<script src="/js/views/json-editor-view-demodesk.es6.js"></script>
<script src="/js/views/login-view.es6.js"></script>
<script src="/js/views/schedule-view.es6.js"></script>
<script src="/js/views/upload-form-view.es6.js"></script>
<script src="/js/app/main-menu-toggle.es6.js"></script>
<script src="/js/app/tracking.es6.js"></script>
<script src="/js/app/app.es6.js"></script>