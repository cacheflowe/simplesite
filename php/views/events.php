...
<div data-view-type="ScheduleView" data-page-title="<?php global $metadata; echo $metadata->get_pageTitle(); ?>">
  ...
  <div class="row column-headers">
    ...
    <div class="one columns">Day</div>
    <div class="two columns">Start</div>
    <div class="two columns">End</div>
    <div class="three columns">Title</div>
  ...
  <form id="events-form" action="/events/submit" target="_blank" method="POST">
    ...
    if(count($events) > 0) {
      ...
      ?>
        <div class="row">
          ...
          <input class="three columns" type="text" data-type="title" name="title-<?php echo($formId); ?>-" id="title-<?php echo($formId); ?>" placeholder="Event Title" value='<?php echo $dateItem["title"]; ?>'>
          <button class="two columns btn-delete" type="submit" data-action="delete"></button>
        </div>
      <?php
        $index++;
      }
      print('<button type="submit" class="btn-update button-update">Update</button>');
    } else {
    ...
    <div class="row">
      ...
      <input class="three columns" type="text" data-type="title" name="title-<?php echo($formId); ?>" id="title-<?php echo($formId); ?>" placeholder="New Event Title" value=''>
      <button class="two columns btn-add button-update" type="submit"></button>
    </div>
  </form>
</div>
